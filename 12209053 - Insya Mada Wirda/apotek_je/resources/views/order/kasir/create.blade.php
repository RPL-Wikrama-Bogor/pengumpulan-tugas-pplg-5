...
@section('content')
    <form action="{{ route('kasir.order.store') }}" class="card p-4 mt-5" method="POST">
        ...
        <div class="mb-3 row">
            <label for="name_costumer" class="col-sm-2 col-form-label @error('name_costumer') is-invalid @enderror">Nama
                Pembeli</label>
            <div class="col-sm-10">
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                @error('medicines')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
                <select name="medicinies[]" class="form-control" required>
...
@push('script')
    <script>
        ...
        function addSelect() {
            let el = `<select name="medicinies[]" class="form-control mt-3" required>
